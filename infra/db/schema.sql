-- =============================================================================
-- NetGeo — Skema PostgreSQL  [LEGACY / DIBEKUKAN]
-- =============================================================================
-- ⚠️ LEGACY: file ini adalah model "topologi sederhana" (MASTER_SPEC §4) dan
-- BUKAN lagi sumber kebenaran. Skema otoritatif = migrasi berurutan di
-- infra/db/postgres/migrations/ (NetGeo Enterprise ERD). Lihat infra/db/README.md.
-- File ini TIDAK lagi di-bootstrap oleh docker-compose; dipertahankan hanya
-- sebagai acuan DDL untuk backend store legacy hingga store dipindah ke skema baru.
-- =============================================================================
-- NetGeo — Skema PostgreSQL (Single Source of Truth untuk lapisan data)
-- =============================================================================
-- Selaras dengan MASTER_SPEC.md §4 (Model Data Inti).
-- Target: PostgreSQL 16+.
--
-- Prinsip desain:
--   * Identitas pakai UUID v4 (generate di server) -> aman untuk sistem
--     terdistribusi, tidak membocorkan jumlah baris, ramah untuk client yang
--     membuat objek optimistik di canvas sebelum round-trip ke server.
--   * Topologi & config disimpan sebagai JSONB (kolom *_meta / content) supaya
--     fleksibel mengikuti evolusi model graph, tetap bisa di-index (GIN).
--   * Versioning: Project.version (integer optimistic-lock) +
--     ConfigArtifact bersifat append-only (riwayat generate config).
--   * Referential integrity penuh: FK + ON DELETE rules eksplisit.
--   * Enum pakai PostgreSQL ENUM type -> validasi di level DB, hemat storage.
--
-- File ini idempotent untuk objek skema (IF NOT EXISTS) tetapi ENUM dibungkus
-- DO-block agar aman dijalankan ulang.
-- =============================================================================

-- -----------------------------------------------------------------------------
-- Ekstensi
-- -----------------------------------------------------------------------------
CREATE EXTENSION IF NOT EXISTS "pgcrypto";   -- gen_random_uuid()
CREATE EXTENSION IF NOT EXISTS "citext";     -- case-insensitive email/username

-- Skema khusus aplikasi (memisahkan dari objek bawaan public bila perlu).
CREATE SCHEMA IF NOT EXISTS netgeo;
SET search_path TO netgeo, public;

-- -----------------------------------------------------------------------------
-- Enumerasi (mengacu langsung ke §4)
-- -----------------------------------------------------------------------------
DO $$
BEGIN
  -- Node.kind
  IF NOT EXISTS (SELECT 1 FROM pg_type WHERE typname = 'node_kind') THEN
    CREATE TYPE node_kind AS ENUM
      ('router','switch','host','ap','olt','firewall','server');
  END IF;

  -- Node.nos (network operating system). 'forgeos' = NOS deklaratif NetGeo (§5).
  IF NOT EXISTS (SELECT 1 FROM pg_type WHERE typname = 'nos_kind') THEN
    CREATE TYPE nos_kind AS ENUM
      ('forgeos','ios','iosxr','nxos','junos','eos','routeros','vyos','sros','frr','vrp');
  END IF;

  -- Node.mode: simulasi ringan vs emulasi container akurat.
  IF NOT EXISTS (SELECT 1 FROM pg_type WHERE typname = 'node_mode') THEN
    CREATE TYPE node_mode AS ENUM ('sim','emul');
  END IF;

  -- Node.status: lifecycle runtime node.
  IF NOT EXISTS (SELECT 1 FROM pg_type WHERE typname = 'node_status') THEN
    CREATE TYPE node_status AS ENUM
      ('stopped','booting','running','degraded','error');
  END IF;

  -- Interface.type
  IF NOT EXISTS (SELECT 1 FROM pg_type WHERE typname = 'iface_type') THEN
    CREATE TYPE iface_type AS ENUM
      ('eth','sfp','sfp28','qsfp','gpon','wifi');
  END IF;

  -- Link.type
  IF NOT EXISTS (SELECT 1 FROM pg_type WHERE typname = 'link_type') THEN
    CREATE TYPE link_type AS ENUM
      ('copper','fiber','wireless','virtual');
  END IF;

  -- ConfigArtifact.format
  IF NOT EXISTS (SELECT 1 FROM pg_type WHERE typname = 'config_format') THEN
    CREATE TYPE config_format AS ENUM ('cli','netconf','yaml');
  END IF;

  -- User.role (RBAC dasar).
  IF NOT EXISTS (SELECT 1 FROM pg_type WHERE typname = 'user_role') THEN
    CREATE TYPE user_role AS ENUM ('owner','admin','editor','viewer');
  END IF;
END$$;

-- -----------------------------------------------------------------------------
-- Trigger helper: auto-update kolom updated_at
-- -----------------------------------------------------------------------------
CREATE OR REPLACE FUNCTION set_updated_at()
RETURNS TRIGGER AS $$
BEGIN
  NEW.updated_at = now();
  RETURN NEW;
END;
$$ LANGUAGE plpgsql;

-- =============================================================================
-- 1. USER  (pemilik project, auth & RBAC)
--    §4 menyebut User sebagai entitas yang dikelola DB. Dimodelkan minimal-tapi-
--    lengkap untuk autentikasi dan kepemilikan project.
-- =============================================================================
CREATE TABLE IF NOT EXISTS app_user (
    id              UUID         PRIMARY KEY DEFAULT gen_random_uuid(),
    username        CITEXT       NOT NULL UNIQUE,
    email           CITEXT       NOT NULL UNIQUE,
    password_hash   TEXT         NOT NULL,             -- argon2/bcrypt, dihitung di backend
    display_name    TEXT,
    is_active       BOOLEAN      NOT NULL DEFAULT TRUE,
    preferences     JSONB        NOT NULL DEFAULT '{}'::jsonb,  -- tema UI, bahasa (i18n)
    created_at      TIMESTAMPTZ  NOT NULL DEFAULT now(),
    updated_at      TIMESTAMPTZ  NOT NULL DEFAULT now(),

    CONSTRAINT app_user_email_chk CHECK (position('@' IN email) > 1)
);

DROP TRIGGER IF EXISTS trg_app_user_updated ON app_user;
CREATE TRIGGER trg_app_user_updated
    BEFORE UPDATE ON app_user
    FOR EACH ROW EXECUTE FUNCTION set_updated_at();

-- =============================================================================
-- 2. PROJECT  (§4: id, name, description, version, topology_ref, created_at)
--    topology_ref disimpan sebagai JSONB (graph nodes/links serializable, §2).
--    version = optimistic-lock counter; naik tiap mutasi topologi.
-- =============================================================================
CREATE TABLE IF NOT EXISTS project (
    id              UUID         PRIMARY KEY DEFAULT gen_random_uuid(),
    owner_id        UUID         NOT NULL
                                 REFERENCES app_user(id) ON DELETE RESTRICT,
    name            TEXT         NOT NULL,
    description     TEXT,
    version         INTEGER      NOT NULL DEFAULT 1,   -- optimistic concurrency
    -- Snapshot/pointer topologi graph. Bisa berisi metadata layout canvas,
    -- viewport, atau referensi eksternal. Sumber-kebenaran node/link tetap di
    -- tabel relasional; topology_ref untuk hal yang murni presentasional/meta.
    topology_ref    JSONB        NOT NULL DEFAULT '{}'::jsonb,
    created_at      TIMESTAMPTZ  NOT NULL DEFAULT now(),
    updated_at      TIMESTAMPTZ  NOT NULL DEFAULT now(),

    CONSTRAINT project_name_owner_uq UNIQUE (owner_id, name),
    CONSTRAINT project_version_pos   CHECK (version >= 1)
);

CREATE INDEX IF NOT EXISTS idx_project_owner    ON project(owner_id);
CREATE INDEX IF NOT EXISTS idx_project_topology ON project USING GIN (topology_ref);

DROP TRIGGER IF EXISTS trg_project_updated ON project;
CREATE TRIGGER trg_project_updated
    BEFORE UPDATE ON project
    FOR EACH ROW EXECUTE FUNCTION set_updated_at();

-- Keanggotaan project (kolaborasi multi-user, RBAC per-project).
CREATE TABLE IF NOT EXISTS project_member (
    project_id      UUID         NOT NULL REFERENCES project(id) ON DELETE CASCADE,
    user_id         UUID         NOT NULL REFERENCES app_user(id) ON DELETE CASCADE,
    role            user_role    NOT NULL DEFAULT 'viewer',
    added_at        TIMESTAMPTZ  NOT NULL DEFAULT now(),
    PRIMARY KEY (project_id, user_id)
);

CREATE INDEX IF NOT EXISTS idx_member_user ON project_member(user_id);

-- =============================================================================
-- 3. NODE  (§4: id, name, kind, nos, mode, x, y, interfaces[], config_ref, status)
--    interfaces[] = relasi 1:N ke tabel interface (bukan array kolom).
--    config_ref   = pointer ke ConfigArtifact aktif (nullable).
-- =============================================================================
CREATE TABLE IF NOT EXISTS node (
    id              UUID         PRIMARY KEY DEFAULT gen_random_uuid(),
    project_id      UUID         NOT NULL REFERENCES project(id) ON DELETE CASCADE,
    name            TEXT         NOT NULL,
    kind            node_kind    NOT NULL,
    nos             nos_kind     NOT NULL DEFAULT 'forgeos',
    mode            node_mode    NOT NULL DEFAULT 'sim',
    x               DOUBLE PRECISION NOT NULL DEFAULT 0,   -- posisi canvas
    y               DOUBLE PRECISION NOT NULL DEFAULT 0,
    status          node_status  NOT NULL DEFAULT 'stopped',
    -- Pointer ke artefak config yang sedang dipakai node (versi aktif).
    -- ON DELETE SET NULL: hapus artefak tidak menghapus node.
    config_ref      UUID,
    -- Atribut device-spesifik (model hardware, image emul, resource limit, dll).
    -- Diisi mengikuti hardware lib (area telecom-network-hardware-architect).
    attributes      JSONB        NOT NULL DEFAULT '{}'::jsonb,
    created_at      TIMESTAMPTZ  NOT NULL DEFAULT now(),
    updated_at      TIMESTAMPTZ  NOT NULL DEFAULT now(),

    CONSTRAINT node_name_project_uq UNIQUE (project_id, name)
);

CREATE INDEX IF NOT EXISTS idx_node_project    ON node(project_id);
CREATE INDEX IF NOT EXISTS idx_node_status     ON node(project_id, status);
CREATE INDEX IF NOT EXISTS idx_node_attributes ON node USING GIN (attributes);

DROP TRIGGER IF EXISTS trg_node_updated ON node;
CREATE TRIGGER trg_node_updated
    BEFORE UPDATE ON node
    FOR EACH ROW EXECUTE FUNCTION set_updated_at();

-- =============================================================================
-- 4. INTERFACE  (§4: id, node_id, name, type, ip[], mac, speed, mtu, peer_link_id)
--    ip[]         = INET[] (mendukung IPv4 & IPv6, sesuai §1 IPv6/SRv6).
--    peer_link_id = link yang menghubungkan interface ini (nullable bila down/unwired).
-- =============================================================================
CREATE TABLE IF NOT EXISTS iface (
    id              UUID         PRIMARY KEY DEFAULT gen_random_uuid(),
    node_id         UUID         NOT NULL REFERENCES node(id) ON DELETE CASCADE,
    name            TEXT         NOT NULL,            -- mis. "GigabitEthernet0/1", "eth0"
    type            iface_type   NOT NULL DEFAULT 'eth',
    ip              INET[]       NOT NULL DEFAULT '{}',   -- v4+v6
    mac             MACADDR,                          -- nullable utk iface logical
    speed_mbps      BIGINT,                           -- kapasitas, mis. 100000 = 100G
    mtu             INTEGER      NOT NULL DEFAULT 1500,
    peer_link_id    UUID,                             -- FK di-add setelah tabel link (siklik)
    attributes      JSONB        NOT NULL DEFAULT '{}'::jsonb,
    created_at      TIMESTAMPTZ  NOT NULL DEFAULT now(),
    updated_at      TIMESTAMPTZ  NOT NULL DEFAULT now(),

    CONSTRAINT iface_name_node_uq UNIQUE (node_id, name),
    CONSTRAINT iface_mtu_chk      CHECK (mtu BETWEEN 64 AND 65535),
    CONSTRAINT iface_speed_chk    CHECK (speed_mbps IS NULL OR speed_mbps > 0)
);

CREATE INDEX IF NOT EXISTS idx_iface_node ON iface(node_id);
CREATE INDEX IF NOT EXISTS idx_iface_peer ON iface(peer_link_id);

DROP TRIGGER IF EXISTS trg_iface_updated ON iface;
CREATE TRIGGER trg_iface_updated
    BEFORE UPDATE ON iface
    FOR EACH ROW EXECUTE FUNCTION set_updated_at();

-- =============================================================================
-- 5. LINK  (§4: id, a_iface, b_iface, type, bandwidth, delay, loss, mtu)
--    Link menghubungkan tepat 2 interface (point-to-point). Karakteristik
--    medium (bandwidth/delay/loss) dipakai engine simulasi discrete-event.
-- =============================================================================
CREATE TABLE IF NOT EXISTS link (
    id              UUID         PRIMARY KEY DEFAULT gen_random_uuid(),
    project_id      UUID         NOT NULL REFERENCES project(id) ON DELETE CASCADE,
    a_iface         UUID         NOT NULL REFERENCES iface(id) ON DELETE CASCADE,
    b_iface         UUID         NOT NULL REFERENCES iface(id) ON DELETE CASCADE,
    type            link_type    NOT NULL DEFAULT 'copper',
    bandwidth_mbps  BIGINT,                           -- kapasitas link efektif
    delay_ms        DOUBLE PRECISION NOT NULL DEFAULT 0,
    loss_pct        DOUBLE PRECISION NOT NULL DEFAULT 0,  -- 0..100
    mtu             INTEGER      NOT NULL DEFAULT 1500,
    attributes      JSONB        NOT NULL DEFAULT '{}'::jsonb,
    created_at      TIMESTAMPTZ  NOT NULL DEFAULT now(),
    updated_at      TIMESTAMPTZ  NOT NULL DEFAULT now(),

    -- Sebuah interface tak boleh memakai dua ujung link yang sama, dan link
    -- harus menghubungkan dua interface berbeda.
    CONSTRAINT link_distinct_iface CHECK (a_iface <> b_iface),
    CONSTRAINT link_loss_chk       CHECK (loss_pct BETWEEN 0 AND 100),
    CONSTRAINT link_delay_chk      CHECK (delay_ms >= 0),
    CONSTRAINT link_mtu_chk        CHECK (mtu BETWEEN 64 AND 65535),
    CONSTRAINT link_bw_chk         CHECK (bandwidth_mbps IS NULL OR bandwidth_mbps > 0),
    -- Normalisasi pasangan agar (A,B) dan (B,A) dianggap link yang sama -> cegah duplikat.
    CONSTRAINT link_pair_uq        UNIQUE (a_iface, b_iface)
);

CREATE INDEX IF NOT EXISTS idx_link_project ON link(project_id);
CREATE INDEX IF NOT EXISTS idx_link_a       ON link(a_iface);
CREATE INDEX IF NOT EXISTS idx_link_b       ON link(b_iface);

DROP TRIGGER IF EXISTS trg_link_updated ON link;
CREATE TRIGGER trg_link_updated
    BEFORE UPDATE ON link
    FOR EACH ROW EXECUTE FUNCTION set_updated_at();

-- Tutup siklus FK: iface.peer_link_id -> link.id (deferred-safe).
-- ON DELETE SET NULL: hapus link, iface tetap ada (cuma jadi unwired).
ALTER TABLE iface
    DROP CONSTRAINT IF EXISTS iface_peer_link_fk;
ALTER TABLE iface
    ADD CONSTRAINT iface_peer_link_fk
    FOREIGN KEY (peer_link_id) REFERENCES link(id) ON DELETE SET NULL;

-- =============================================================================
-- 6. SCENARIO  (§4: id, project_id, name, steps[], expected_outcomes[])
--    steps & expected_outcomes = JSONB array (urutan langkah uji + hasil harapan).
-- =============================================================================
CREATE TABLE IF NOT EXISTS scenario (
    id                  UUID         PRIMARY KEY DEFAULT gen_random_uuid(),
    project_id          UUID         NOT NULL REFERENCES project(id) ON DELETE CASCADE,
    name                TEXT         NOT NULL,
    description         TEXT,
    steps               JSONB        NOT NULL DEFAULT '[]'::jsonb,
    expected_outcomes   JSONB        NOT NULL DEFAULT '[]'::jsonb,
    created_at          TIMESTAMPTZ  NOT NULL DEFAULT now(),
    updated_at          TIMESTAMPTZ  NOT NULL DEFAULT now(),

    CONSTRAINT scenario_name_project_uq UNIQUE (project_id, name),
    CONSTRAINT scenario_steps_is_array  CHECK (jsonb_typeof(steps) = 'array'),
    CONSTRAINT scenario_outcomes_array  CHECK (jsonb_typeof(expected_outcomes) = 'array')
);

CREATE INDEX IF NOT EXISTS idx_scenario_project ON scenario(project_id);
CREATE INDEX IF NOT EXISTS idx_scenario_steps   ON scenario USING GIN (steps);

DROP TRIGGER IF EXISTS trg_scenario_updated ON scenario;
CREATE TRIGGER trg_scenario_updated
    BEFORE UPDATE ON scenario
    FOR EACH ROW EXECUTE FUNCTION set_updated_at();

-- =============================================================================
-- 7. CONFIG_ARTIFACT  (§4: id, node_id, vendor, format, content, generated_at)
--    APPEND-ONLY: tiap generate menghasilkan baris baru (riwayat versi config).
--    Versi aktif ditandai is_active + dirujuk node.config_ref.
-- =============================================================================
CREATE TABLE IF NOT EXISTS config_artifact (
    id              UUID          PRIMARY KEY DEFAULT gen_random_uuid(),
    node_id         UUID          NOT NULL REFERENCES node(id) ON DELETE CASCADE,
    vendor          nos_kind      NOT NULL,           -- target NOS (sejajar Node.nos)
    format          config_format NOT NULL DEFAULT 'cli',
    -- intent ForgeOS sumber (§5) -> jejak audit "satu intent → banyak target NOS".
    source_intent   JSONB,
    content         TEXT          NOT NULL,           -- config ter-render (CLI/NETCONF/YAML)
    version         INTEGER       NOT NULL DEFAULT 1, -- nomor revisi per-node
    is_active       BOOLEAN       NOT NULL DEFAULT FALSE,
    generated_at    TIMESTAMPTZ   NOT NULL DEFAULT now(),

    CONSTRAINT config_version_pos CHECK (version >= 1)
);

CREATE INDEX IF NOT EXISTS idx_config_node ON config_artifact(node_id, generated_at DESC);
-- Hanya boleh ada SATU config aktif per node (partial unique index).
CREATE UNIQUE INDEX IF NOT EXISTS uq_config_active_per_node
    ON config_artifact(node_id) WHERE is_active;

-- Tutup siklus FK: node.config_ref -> config_artifact.id.
ALTER TABLE node
    DROP CONSTRAINT IF EXISTS node_config_ref_fk;
ALTER TABLE node
    ADD CONSTRAINT node_config_ref_fk
    FOREIGN KEY (config_ref) REFERENCES config_artifact(id) ON DELETE SET NULL;

-- =============================================================================
-- 8. SIMULATION_RUN  (pendukung /api/simulate — riwayat eksekusi engine)
--    Bukan di §4 secara eksplisit, tapi diperlukan agar status job (yang state
--    live-nya di Redis) punya catatan persisten/audit di Postgres.
-- =============================================================================
CREATE TABLE IF NOT EXISTS simulation_run (
    id              UUID          PRIMARY KEY DEFAULT gen_random_uuid(),
    project_id      UUID          NOT NULL REFERENCES project(id) ON DELETE CASCADE,
    scenario_id     UUID          REFERENCES scenario(id) ON DELETE SET NULL,
    triggered_by    UUID          REFERENCES app_user(id) ON DELETE SET NULL,
    status          TEXT          NOT NULL DEFAULT 'queued',  -- queued|running|done|failed|cancelled
    -- Hasil ringkas / metrik (detail live ada di Redis, lihat redis-design.md).
    result          JSONB         NOT NULL DEFAULT '{}'::jsonb,
    started_at      TIMESTAMPTZ,
    finished_at     TIMESTAMPTZ,
    created_at      TIMESTAMPTZ   NOT NULL DEFAULT now(),

    CONSTRAINT simrun_status_chk
        CHECK (status IN ('queued','running','done','failed','cancelled'))
);

CREATE INDEX IF NOT EXISTS idx_simrun_project ON simulation_run(project_id, created_at DESC);

-- =============================================================================
-- Views bantu (read-model untuk backend)
-- =============================================================================

-- Topologi lengkap per project (untuk endpoint GET /api/projects/{id}/topology
-- & WS /ws/topology bootstrap).
CREATE OR REPLACE VIEW v_project_topology AS
SELECT
    p.id              AS project_id,
    p.name            AS project_name,
    p.version         AS project_version,
    (SELECT count(*) FROM node n WHERE n.project_id = p.id) AS node_count,
    (SELECT count(*) FROM link l WHERE l.project_id = p.id) AS link_count
FROM project p;

-- =============================================================================
-- Seed minimal (opsional, untuk dev). Hapus di prod.
-- =============================================================================
-- INSERT INTO app_user (username, email, password_hash, display_name)
-- VALUES ('admin','admin@netgeo.local','<argon2-hash>','NetGeo Admin')
-- ON CONFLICT DO NOTHING;

-- =============================================================================
-- Selesai. Jalankan via migrations/ (lihat infra/db/migrations/).
-- =============================================================================
